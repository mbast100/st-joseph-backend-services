...
def media():
    ...
    if request.method == "GET":
        s3.list_files_by_prefix(args.get("prefix")) if args.get(
            "prefix") else s3.list_files(args.get("bucket"))

        if s3.contents:
            return jsonify(s3.contents), 200
        else:
            return jsonify(s3.error), 400

    elif request.method == "POST":
        try:
            file_to_upload = request.files["image"]
            if args.get("type") == "seasonal":
                s3.upload_file("st-joseph-seasonal", file_to_upload.filename,
                               file_to_upload, path=args.get("month"))
                return jsonify({
                ...
                s3.upload_file("st-joseph-media", file_to_upload.filename,
                               file_to_upload, path=args.get("type"))
                return jsonify({
